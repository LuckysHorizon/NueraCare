...
export default function NotificationsPermissionScreen() {
  ...
  const payload = useMemo(
    () => ({
      selectedOptions: ["Notifications permission: requested"],
      optionalNote: null,
    }),
    []
  );
  ...
  const handleNext = (decision: "Allow" | "Skip") => {
    console.log("Notifications permission", { ...payload, decision });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={["#F5FCFB", "#FFFFFF"]} style={styles.gradientContainer}>
        <OnboardingHeader step={10} totalSteps={12} title="Notifications" />
        <ScrollView
          contentContainerStyle={[styles.content, styles.contentGrow]}
          showsVerticalScrollIndicator={false}
        >
          <Heading level={2} style={styles.title}>Notifications</Heading>
          <Body style={styles.subtitle}>
            Lets us send gentle reminders and updates you request.
          </Body>
        </ScrollView>
